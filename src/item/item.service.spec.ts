...
describe('ItemService', () => {
  ...
  describe('find', () => {
    it('retrieves the specified item from the table', () => {
      ...
      const itemToRetrieve = items.filter(({ id }) => id === idOfItemToRetrieve)[0];

      expect(itemService.find(idOfItemToRetrieve)).resolves.toEqual(itemToRetrieve);
    })
  })
  ...
  describe('create', () => {
    it('creates an item with the specified title', async () => {
      ...
      const newItemExists = await itemsRepository.exist({ where: { title: newItemTitle } })
  ...
  describe('update', () => {
    it('updates an item', async () => {
      ...
      const updateOperationResult = await itemService.update(expectedUpdatedItem.id, expectedUpdatedItem.title);
      const updatedItem = await itemsRepository.findOneBy({ id: expectedUpdatedItem.id });
  ...
  describe('delete', ()  => {
    it('deletes an item', async () => {
      ...
      const deletedItemExists = await itemsRepository.exist({ where: { id: idOfItemToDelete } })
